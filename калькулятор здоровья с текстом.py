...
if page == "Жировой запас":
    ...
    st.header(" Описание процедуры снятия показателей ")
    st.markdown(" Для расчёта индекса массы тела нужно ввести Ваш рост в сантиметрах и массу тела в килограммах в соответствующие поля. ")
    st.header(" Методика расчёта результатов")
    st.markdown(" Индекс массы тела рассчитывается по формуле Кетле как отношение массы тела в килограммах к квадрату роста в метрах: ИМТ= масса тела/ (рост*рост), затем соотносится с нормой для введённого веса. ")
    st.header(" Рекомендации ")
    st.markdown(" При отклонении ИМТ от нормы рекомендуем Вам обратиться к специалисту и скорректировать диету, физические нагрузки и другие факторы, влияющие на массу тела. ")
    st.markdown(" Тест не равноценен медицинскому исследованию. Точно интерпретировать результаты пробы может специалист. ")
    ...
    st.header("""Сердце:""")
    st.header(" Описание процедуры снятия показателей ")
    st.markdown("1. Убедитесь, что в течение 20 минут до измерения пульса Вы не занимались физической активностью и находились в состоянии покоя.")
    st.markdown("2. Положите кончики указательного, среднего и безымянного пальцев на внутреннюю сторону запястья со стороны большого пальца кнутри от кости. Осторожно надавите, нащупайте пульс.")
    st.markdown("3. Измеряйте пульс в течение 60 секунд.")
    st.header(" Желательность результатов ")
    st.markdown(" По данным Американской Кардиологической Ассоциации, нормальное значение частоты сердечных сокращений (ЧСС) в состоянии покоя находится в диапазоне 60-100 ударов в минуту для людей старше 15 лет. ")
    st.markdown(" Для спортсменов и людей, принимающих бета - адреноблокаторы характерен более низкий показатель ЧСС. ")
    st.header("Рекомендации")
    ...
    st.markdown("Для мужчин удовлетворительное время задержки составляет от 35 до 49, хороший результат — от 50 до 57, отличный — 58 и более. ")
    st.markdown("Для женщин удовлетворительно время задержки от 21 до 31 секунды, хорошо — от 32 до 37, отлично — 38 и более. ")
    st.header("Рекомендации")
